Umbrella2d <- function(datalist,vol=0.85,NTC=NULL,Plate=T,Rotate=F,method="argam"){
...
return(list(fdr=fdr,datafit=fmarg,datanull=f0,pifits=pi0,res=res,model=fitHlp))
}
...
res <- c(mu,NTC$NTCsig,prob)
return(list(fdr=fdr,datafit=fmarg,datanull=f0,pifits=pi0,res=res,model=b))
}
...
Shellfunc <- function(data2d,modout,method){
	time1 <- Sys.time()
...
# initial fit on full data
	if((!is.null(NTCxN))&(!is.null(NTCyN))){
		...
		if(method=="cgam"){
		try(Locoutx <- lapply(as.list(1:length(NTCxN)),function(i){Locfunc3(dataxs,modout$center[1],NTCxN[[i]])}))
		try(Locouty <- lapply(as.list(1:length(NTCyN)),function(i){Locfunc3(datays,modout$center[2],NTCyN[[i]])}))
			# if NTC's are present and method=cgam, use Locfunc3 for fitting
			}
		if(method=="scam"){
		try(Locoutx <- lapply(as.list(1:length(NTCxN)),function(i){Locfunc4(dataxs,modout$center[1],NTCxN[[i]])}))
		try(Locouty <- lapply(as.list(1:length(NTCyN)),function(i){Locfunc4(datays,modout$center[2],NTCyN[[i]])}))
	...
	Ttime2 <- Sys.time()
		...
		if(method=="cgam" | method=="scam"){
			try(probx <- probmx[rax])
			try(proby <- probmy[ray])
			}
		} # end NTC procedure
...
# NTC version fit
	if((!is.null(NTCxN))&(!is.null(NTCyN))){
		...
		if(length(datax0)>20 & length(datax1)>20){
			...
			if(method=="cgam"){
			try(Locoutx0 <- lapply(as.list(1:length(NTCxN)),function(i){Locfunc3(sort(datax0),modout$center[1],NTCxN[[i]])}))
				}
			if(method=="scam"){
			try(Locoutx0 <- lapply(as.list(1:length(NTCxN)),function(i){Locfunc4(sort(datax0),modout$center[1],NTCxN[[i]])}))
				}
			...
			try(fitmodelx <- lapply(Locoutx0,function(x){x$model}))
			...
			datax1c <- sort((datax1-modout$center[1])/sigdifx+modout$center[1])
				# difference of variability of the null cluster

			if(method=="argam"){
				fdrfunc <- function(x,prob,sigma,mu){
					fmarg <- predict(x,data.frame(mids=datax1c-mu,dummy=1),type="response")
					f0 <- prob*predict(x,data.frame(mids=datax1c-mu,dummy=0),type="response")
					fdr <- f0/fmarg
					fdr[(datax1c-mu) < -sigma] <- 1
					fdr}
				f0func <- function(x,prob,sigma,mu){
					f0 <- prob*predict(x,data.frame(mids=datax1c-mu,dummy=0),type="response")
					f0}
				fmfunc <- function(x,prob,sigma,mu){
					fmarg <- predict(x,data.frame(mids=datax1c-mu,dummy=1),type="response")
					fmarg}
				}
			if(method=="cgam" | method=="scam"){
				fdrfunc <- function(x,prob,sigma,mu){
					fmarg <- predict(x,data.frame(mids=datax1c-mu,mids2=datax1c-mu,dummy=1,dummySmooth=as.double((datax1c-mu)>= -sigma)),type="response")
					f0 <- predict(x,data.frame(mids=datax1c-mu,mids2=datax1c-mu,dummy=0,dummySmooth=0),type="response")
					fdr <- prob*f0/fmarg
					fdr}
				f0func <- function(x,prob,sigma,mu){
					f0 <- predict(x,data.frame(mids=datax1c-mu,mids2=datax1c-mu,dummy=0,dummySmooth=0),type="response")
			...
			try(problistx1 <- lapply(as.list(1:length(NTCxN)),function(i){fdrfunc(fitmodelx[[i]],pix[[i]],sigx[[i]],mux[[i]])}))
			try(datafitx1 <- lapply(as.list(1:length(NTCxN)),function(i){fmfunc(fitmodelx[[i]],pix[[i]],sigx[[i]],mux[[i]])[rax1]}))
			try(ntcfitx1 <- lapply(as.list(1:length(NTCxN)),function(i){f0func(fitmodelx[[i]],pix[[i]],sigx[[i]],mux[[i]])[rax1]}))

			try(datafitx <- mapply(c,lapply(Locoutx0,function(x){x$datafit[rax0]}),datafitx1,SIMPLIFY=F))
			try(datafitx <- lapply(datafitx,function(x){x[posx]}))
				# store data fit sorted as in original data
			try(ntcfitx <- mapply(c,lapply(Locoutx0,function(x){x$datanull[rax0]}),ntcfitx1,SIMPLIFY=F))
...
Ttime4[[1]] <- Sys.time()
			...
			if(method=="cgam" | method=="scam"){
				try(probx0 <- probmx0[rax0])
				try(probx1 <- probmx1[rax1])
				}
			...
			if(method=="cgam" | method=="scam"){
				probciLx0 <- pmax(probciLx0,0)[rax0]
				probciUx0 <- pmin(probciUx0,1)[rax0]
				probciLx1 <- pmax(probciLx1,0)[rax1]
				probciUx1 <- pmin(probciUx1,1)[rax1]
			}
...
Ttime4[[2]] <- Sys.time()
			...
			if(method=="cgam" | method=="scam"){
				probciLx <- pmax(probciLx[rax],0)
				probciUx <- pmin(probciUx[rax],1)
				}
			}) # confidence limits, if possible
			try(if(length(NTCxN)==1){
				probrx <- probrx[rax]})
	...
	# y-loop
		...
		if(method=="cgam"){
			try(Locouty0 <- lapply(as.list(1:length(NTCyN)),function(i){Locfunc3(sort(datay0),modout$center[2],NTCyN[[i]])}))
		}
		if(method=="scam"){
			try(Locouty0 <- lapply(as.list(1:length(NTCyN)),function(i){Locfunc4(sort(datay0),modout$center[2],NTCyN[[i]])}))
		}
				# use only neg droplets other channels as predictors
			...
			try(fitmodely <- lapply(Locouty0,function(x){x$model}))
			...
			datay1c <- sort((datay1-modout$center[2])/sigdify+modout$center[2])
				# difference of variability of the null cluster

			if(method=="argam"){
				fdrfunc <- function(x,prob,sigma,mu){
					fmarg <- predict(x,data.frame(mids=datay1c-mu,dummy=1),type="response")
					f0 <- prob*predict(x,data.frame(mids=datay1c-mu,dummy=0),type="response")
					fdr <- f0/fmarg
					fdr[(datay1c-mu) < -sigma] <- 1
					fdr}
				f0func <- function(x,prob,sigma,mu){
					f0 <- prob*predict(x,data.frame(mids=datay1c-mu,dummy=0),type="response")
					f0}
			...
			if(method=="cgam" | method=="scam"){
				fdrfunc <- function(x,prob,sigma,mu){
					fmarg <- predict(x,data.frame(mids=datay1c-mu,mids2=datay1c-mu,dummy=1,dummySmooth=as.double((datay1c-mu)>= -sigma)),type="response")
					f0 <- predict(x,data.frame(mids=datay1c-mu,mids2=datay1c-mu,dummy=0,dummySmooth=0),type="response")
					fdr <- prob*f0/fmarg
					fdr}
				f0func <- function(x,prob,sigma,mu){
					f0 <- predict(x,data.frame(mids=datay1c-mu,mids2=datay1c-mu,dummy=0,dummySmooth=0),type="response")
			...
			try(problisty1 <- lapply(as.list(1:length(NTCyN)),function(i){fdrfunc(fitmodely[[i]],piy[[i]],sigy[[i]],muy[[i]])}))
			try(datafity1 <- lapply(as.list(1:length(NTCyN)),function(i){fmfunc(fitmodely[[i]],piy[[i]],sigy[[i]],muy[[i]])[ray1]}))
				# store posterior probabilities
			try(ntcfity1 <- lapply(as.list(1:length(NTCyN)),function(i){f0func(fitmodely[[i]],piy[[i]],sigy[[i]],muy[[i]])[ray1]}))

			try(datafity <- mapply(c,lapply(Locouty0,function(x){x$datafit[ray0]}),datafity1,SIMPLIFY=F))
			try(datafity <- lapply(datafity,function(x){x[posy]}))
				# store data fit sorted as in original data
			try(ntcfity <- mapply(c,lapply(Locouty0,function(x){x$datanull[ray0]}),ntcfity1,SIMPLIFY=F))
...
Ttime4[[3]] <- Sys.time()
		...
		if(method=="cgam" | method=="scam"){
			try(proby0 <- probmy0[ray0])
			try(proby1 <- probmy1[ray1])
			}
		...
		if(method=="cgam" | method=="scam"){
				probciLy0 <- pmax(probciLy0,0)[ray0]
				probciUy0 <- pmin(probciUy0,1)[ray0]
				probciLy1 <- pmax(probciLy1,0)[ray1]
				probciUy1 <- pmin(probciUy1,1)[ray1]
			}
...
Ttime4[[4]] <- Sys.time()
			...
			if(method=="cgam" | method=="scam"){
				probciLy <- pmax(probciLy[ray],0)
				probciUy <- pmin(probciUy[ray],1)
			}
			}) # confidence limits, if possible
			try(if(length(NTCyN)==1){
				probry <- probry[ray]})
...
# write output
	...
	try(concx <- rbind(c(1-px,-log(px)*1000/vol,pposx,-log(1-pposx)*1000/vol),
				...
				c(1-pircix[2],concix[1]*1000/vol,ciposx[1]/ndropx,concposx[1]*1000/vol),
				c(1-pircix[1],concix[2]*1000/vol,ciposx[2]/ndropx,concposx[2]*1000/vol)))
	...
	try(concy <- rbind(c(1-py,-log(py)*1000/vol,pposy,-log(1-pposy)*1000/vol),
				...
				c(1-pirciy[2],conciy[1]*1000/vol,ciposy[1]/ndropy,concposy[1]*1000/vol),
				c(1-pirciy[1],conciy[2]*1000/vol,ciposy[2]/ndropy,concposy[2]*1000/vol)))
...
resultlist <- lapply(as.list(1:length(datarotlist)),function(i){Shellfunc(datarotlist[[i]],modplat[[i]],method)})
