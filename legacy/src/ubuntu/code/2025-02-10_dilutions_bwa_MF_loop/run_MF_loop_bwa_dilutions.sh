#!/bin/bash
# run_MF_loop_bwa_dilutions.sh
#
# This script loops over samples in the /add/MosaicForecast/input/ directory,
# where sample files are named with the suffix "_bwa" (e.g. A100_1to2_bwa.bam, NA100_1to10_bwa.bam, etc.).
#
...
#   /add/results/2025-02-10_MF_loop_bwa_dilutions/
#
...
RESULTS_DIR="/add/results/2025-02-10_MF_loop_bwa_dilutions"
mkdir -p "$RESULTS_DIR"
...
# Loop over BAM files ending with _bwa.bam
for bam_file in "${INPUT_DIR}"/*_bwa.bam; do
    # Extract the sample name (e.g. from A100_1to2_bwa.bam, sample becomes "A100_1to2_bwa")
