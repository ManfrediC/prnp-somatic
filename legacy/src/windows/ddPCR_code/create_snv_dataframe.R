setwd("C:/Users/Manfredi/USZ/Neuropathologie - Carta Manfredi/CJD PRNP/Experiments/ddPCR/CSV files pool")
...
bigdata <- purrr::map_dfr(files, function(f) {
  ...
  df$Date <- as.Date(substr(f, 1, 10), format = "%Y-%m-%d")
  ...
  mut_str <- stringr::str_extract(f, "(?i)(?<=SNV_)(D178N|E200K|P102L)")
...
#also change Parchi sample names to our naming system
data.mut$Sample %<>%
  gsub("-bg", "_bg", .) %>%
  gsub("-cb", "_cb", .) %>%
  gsub("-fr", "_fr", .) %>%
...
#remove pons (not part of this project) - actually, let's keep it
#data.mut <- subset(data.mut, Sample != "15-27_ps")
...
patient.names <- read.xlsx("patient_names.xlsx") %>%
  as_tibble() %>%
...
# dataset
setwd("C:/Users/Manfredi/USZ/Neuropathologie - Carta Manfredi/CJD PRNP/Experiments/ddPCR/final dataset")
write.xlsx(ddpcr.plot.data, "SNV_data_final.xlsx")

# assay-wide fallback p0
write.csv(assay_fallback, "p0_fallback.csv", row.names = FALSE)
...
# export
setwd("C:/Users/Manfredi/USZ/Neuropathologie - Carta Manfredi/CJD PRNP/Experiments/ddPCR/final dataset")
write.xlsx(pooled_plot_data, "SNV_pooled_participant.xlsx")

# also export to paper results directory
setwd("C:/Users/Manfredi/USZ/Neuropathologie - Carta Manfredi/CJD PRNP/Manuscript/Tables/ddPCR_sample_results")
write.xlsx(ddpcr.plot.data, "SNV_data_final.xlsx")
write.xlsx(pooled_plot_data, "SNV_pooled_participant.xlsx")
