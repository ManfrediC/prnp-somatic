...
# Define the input metadata, scratch analysis area, raw database, and outputs.
input_dir <- file.path(project_root, "raw", "ddpcr")
sample_details_path <- file.path(input_dir, "sample_details.xlsx")
scratch_root <- file.path(project_root, "scratch", "ddpcr-new", "analysis")
raw_database_root <- file.path(project_root, "scratch", "ddpcr-new", "raw")
validation_dir <- file.path(scratch_root, "validation")
output_dir <- file.path(scratch_root, "results", "ddPCR")
...
source(file.path(scratch_root, "scripts", "ddpcr_raw_import_helpers.R"))
