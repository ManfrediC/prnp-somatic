...
partition_counts_path <- file.path(analysis_root, "results", "ddPCR", "ddpcr_partition_counts_by_sample_assay.csv")
curated_ddpcr_path <- file.path(analysis_root, "results", "ddPCR", "SNV_data_final.xlsx")
output_dir <- file.path(scratch_root, "tables", "haploid_genomes")
...
if (!file.exists(partition_counts_path)) {
  stop("Missing partition-count input: ", partition_counts_path,
       "\nRun Rscript scratch/ddpcr-new/analysis/scripts/create_snv_dataframe_corrected.R first.")
}
