...
data_path <- file.path(analysis_root, "results", "ddPCR", "SNV_pooled_participant.xlsx")
out_dir <- file.path(scratch_root, "figures", "ddpcr", "pooled_participant")
...
make_plot <- function(mut) {
  ...
  ggplot(df, aes(x = participant, y = fractional_abundance)) +
    ...
    labs(
      title = paste(mut, "corrected participant-pooled ddPCR"),
...
for (mut in mutation_list) {
  ggsave(
    filename = file.path(out_dir, paste0("SNV_pooled_", mut, "_panel_corrected.pdf")),
    plot = plots[[mut]],
    width = 9,
    height = 4.5,
    dpi = 300
...
combined_plot <- ggplot(combined_df, aes(x = participant, y = fractional_abundance)) +
  ...
  labs(
    title = "Corrected participant-pooled ddPCR",
    subtitle = "Pooled fractional abundance, confidence intervals, and LoB are on the same percent scale as LoD",
    x = "Participant",
...
ggsave(
  filename = file.path(out_dir, "SNV_pooled_all_mutations_corrected_single_page.pdf"),
  plot = combined_plot,
  width = 9,
  height = 12,
  dpi = 300
...
write_multipage_pdf(
  file.path(out_dir, "SNV_pooled_all_mutations_corrected_multipage.pdf"),
  plots,
  width = 9,
  height = 4.5
)
...
readr::write_csv(
  pooled_plot_data,
  file.path(out_dir, "participant_pooled_rows_corrected.csv")
)

readr::write_csv(
  pass_rows,
  file.path(out_dir, "participant_pooled_lob_lod_pass_rows_corrected.csv")
)
