...
# Build the complete sequencing metrics manuscript table.
...
## ---- configure inputs and generated table output ----
...
table_tex <- Sys.getenv(
  "PRNP_SEQUENCING_METRICS_TEX",
  unset = file.path(repo_root, "manuscript", "tables", "main", "table_sequencing_metrics.tex")
)
...
## ---- write the complete canonical manuscript table ----
table_lines <- c(
  "% Main Table 5: generated by src/sequencing_qc/make_sequencing_metrics_table_rows.R.",
  "\\begin{table}[t]",
  "  \\centering",
  "  \\resizebox{\\textwidth}{!}{%",
...
if (!file.exists(table_tex) || is.na(file.info(table_tex)$size) || file.info(table_tex)$size <= 0) {
  stop("Canonical sequencing metrics TeX is missing or empty: ", table_tex)
}
