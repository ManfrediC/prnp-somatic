...
# -------------------------------------------------------------------------
# Write the complete canonical supplementary manuscript table.
# -------------------------------------------------------------------------
...
write_latex_table <- function(table, path) {
  ...
  # Escape cells before writing the publication table.
  escaped <- as.data.frame(lapply(table, latex_escape), stringsAsFactors = FALSE)
  lines <- c(
    "% Auto-generated by src/dna_quality/build_dna_quality_evidence_table.R",
    "\\begin{table}[t]",
    "  \\centering",
    "  \\resizebox{\\textwidth}{!}{%",
  ...
  # Add one LaTeX row per sample, separating case and control blocks.
  for (i in seq_len(nrow(escaped))) {
    if (i > 1L && startsWith(as.character(escaped[i, 1]), "Control") &&
        !startsWith(as.character(escaped[i - 1L, 1]), "Control")) {
      lines <- c(lines, "      \\addlinespace")
    }
    lines <- c(lines, paste0("      ", paste(as.character(escaped[i, ]), collapse = " & "), " \\\\"))
  }
  ...
  if (!file.exists(path) || is.na(file.info(path)$size) || file.info(path)$size <= 0) {
    stop("Canonical DNA-quality TeX is missing or empty: ", path)
  }
}
