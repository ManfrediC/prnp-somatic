...
ddpcr_results_dir <- file.path(project_root, "results", "ddPCR")
partition_counts_path <- file.path(ddpcr_results_dir, "ddpcr_partition_counts_by_sample_assay.csv")
curated_ddpcr_path <- file.path(ddpcr_results_dir, "SNV_data_final.xlsx")
output_dir <- file.path(ddpcr_results_dir, "haploid_genomes_surveyed")
...
if (!file.exists(partition_counts_path)) {
  stop("Missing partition-count input: ", partition_counts_path,
       "\nRun Rscript src/ddpcr/create_snv_dataframe.R first.")
}
...
write_basic_tex_from_formatted_table <- function(.data, output_path) {
  ...
  tex_lines <- c(
    ...
    "% Values are generated directly from participant_sample_region_mean_ranges_formatted.csv.",
    "\\begin{landscape}",
    "\\begin{table}[p]",
    "  \\centering",
    ...
    "      \\begin{tabular}{@{}l c r c c c r c c c r c c l@{}}",
    "        \\toprule",
    "        Participant & \\multicolumn{4}{c}{\\dn} & \\multicolumn{4}{c}{\\ek} & \\multicolumn{4}{c}{\\pl} & LoB/LoD pass\\\\",
    "        \\cmidrule(lr){2-5}\\cmidrule(lr){6-9}\\cmidrule(lr){10-13}",
    "        & MUT+ droplets & \\multicolumn{1}{c}{Droplets} & MUT genomes & Total genomes",
    "        & MUT+ droplets & \\multicolumn{1}{c}{Droplets} & MUT genomes & Total genomes",
    "        & MUT+ droplets & \\multicolumn{1}{c}{Droplets} & MUT genomes & Total genomes & \\\\",
    "        \\midrule"
  )

  for (i in seq_len(nrow(tex_cells))) {
    if (i > 1L && startsWith(as.character(tex_cells[i, 1]), "Control") &&
        !startsWith(as.character(tex_cells[i - 1L, 1]), "Control")) {
      tex_lines <- c(tex_lines, "        \\addlinespace[2pt]")
    }
    tex_lines <- c(
      tex_lines,
      paste0("        ", paste(as.character(tex_cells[i, ], use.names = FALSE), collapse = " & "), " \\\\")
    )
  }
  ...
  tex_lines <- c(
    ...
    "    \\end{adjustbox}",
    "  \\endgroup",
    "  \\caption{\\textbf{Participant-level ddPCR droplet counts and estimated haploid genome equivalents.} Values are shown for each participant and mutation assay as mean (range) across available sample-regions after technical pooling. Single-sample summaries omit ranges. Droplet columns report accepted droplets after quality filtering; MUT+ denotes mutant-positive droplets. MUT and total genomes are Poisson estimates from mutant-negative and signal-negative droplet fractions, respectively. LoB/LoD pass lists assays with at least one sample-region passing both limit-of-blank and limit-of-detection filters; None indicates no non-heterozygous assay passed.}",
  ...
  if (!file.exists(output_path) || is.na(file.info(output_path)$size) || file.info(output_path)$size <= 0) {
    stop("Canonical haploid-genome TeX is missing or empty: ", output_path)
  }
}
...
write_basic_tex_from_formatted_table(
  ...
  file.path(project_root, "manuscript", "tables", "supplement", "table_ddpcr_participant_haploid_genomes.tex")
)
...
generated_outputs <- c(
  file.path(output_dir, "sample_region_haploid_genomes.csv"),
  file.path(output_dir, "participant_pooled_haploid_genomes.csv"),
  file.path(project_root, "manuscript", "tables", "supplement", "table_ddpcr_participant_haploid_genomes.tex"),
...
if (length(missing_or_empty) > 0L) {
  stop("Haploid-genome output is missing or empty: ", paste(missing_or_empty, collapse = ", "))
}
