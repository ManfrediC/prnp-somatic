...
write_sample_overview_tex <- function(.data, output_path) {
  ...
  lines <- c(
    ...
    "\\end{tabular}",
    "\\end{adjustbox}",
    "\\caption{\\textbf{Overview of ddPCR single-nucleotide variant analysis.} Purified brain DNA from $31$ CJD and $8$ control individuals was analysed. The table shows the number of sample-regions that passed ddPCR quality control overall and per assay, together with the maximum estimated VAF for \\dn, \\ek and \\pl. VAFs are reported as percentages. The \\textit{LoB/LoD pass} column indicates whether at least one sample-region met both calling criteria: (i) the observed mutant-positive droplet count surpassed the sample-specific LoB count, and (ii) the lower 95\\% CI of the VAF exceeded the assay-specific LoD. ``None'' indicates that no non-germline mosaic call met both criteria. As CJD30 was germline heterozygous for \\ek, it was excluded from \\ek mosaicism summaries.}",
