...
setwd("C:/Users/Manfredi/USZ/Neuropathologie - Carta Manfredi/CJD PRNP/Experiments/SureSelect-sequencing/Analysis/2025-04-19_combined_variant_table_PoN")
...
# annotate gene
summary_table <- summary_table %>%
  mutate(gene = str_extract(FUNCOTATION, "\\b(TTN|PRNP|TET2)\\b"))
summary_table <- summary_table %>%
  relocate(gene, .after = colnames(summary_table)[3])
...
# mutation type
summary_table <- summary_table %>%
  mutate(mutation_type = str_extract(FUNCOTATION, "\\b(SILENT|INTRON|MISSENSE)\\b"))
summary_table <- summary_table %>%
  relocate(mutation_type, .after = colnames(summary_table)[5])
