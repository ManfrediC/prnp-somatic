...
mkdir -p /add/results/2025-03-25_MF_all_bwa_samples/

# Define the output file
output="/add/results/2025-03-25_MF_all_bwa_samples/MF_all_bwa_samples_summary.tsv"
...
# Loop through each sample directory under /add/results/2025-02-10_MF_loop_bwa/ ending with _bwa
for dir in /add/results/2025-02-10_MF_loop_bwa/*_bwa; do
    ...
    if [[ -f "$file" ]]; then
        while read -r line; do
            ...
            # Columns: sample, chr, position, REF, ALT, REF_reads, ALT_reads, conflict_count, variant_type
            ...
            # Calculate VAF with high precision then round to two decimals
            if [[ "$depth" -gt 0 ]]; then
            ...
            # Determine mutation name based on position
            case $pos in
