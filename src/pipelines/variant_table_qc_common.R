...
parse_variant_table_qc_cli <- function(args, sample_manifest_default = NULL) {
  ...
  if (!is.null(sample_manifest_default)) {
  ...
  list(
    ...
    enable_aaf_filter = parse_bool(
      get_arg(parsed, "enable-aaf-filter", "1"),
      "enable-aaf-filter"
    ),
    ...
    thresholds = list(
      min_alt_count = parse_num(
        get_arg(parsed, "min-alt-count", "10"),
        "min-alt-count"
      ),
      min_dp = parse_num(get_arg(parsed, "min-dp", "100"), "min-dp"),
      min_strand_alt = parse_num(
        get_arg(parsed, "min-strand-alt", "3"),
        "min-strand-alt"
      ),
      min_mean_bq = parse_num(
        get_arg(parsed, "min-mean-bq", "20"),
        "min-mean-bq"
      ),
      min_mean_mq = parse_num(
        get_arg(parsed, "min-mean-mq", "20"),
        "min-mean-mq"
      ),
      max_pop_freq = parse_num(
        get_arg(parsed, "max-pop-freq", "0.001"),
        "max-pop-freq"
      ),
      max_binom_p = parse_num(
        get_arg(parsed, "max-binom-p", "1e-6"),
        "max-binom-p"
      )
    )
  )
...
run_variant_table_qc <- function(
  ...
  min_mean_bq <- thresholds$min_mean_bq
  min_mean_mq <- thresholds$min_mean_mq
  max_pop_freq <- thresholds$max_pop_freq
  max_binom_p <- thresholds$max_binom_p

  if (!dir.exists(variant_dir)) {
    fail(paste0("Variant table directory does not exist: ", variant_dir))
  }
  if (!dir.exists(metrics_dir)) {
    fail(paste0("Readcount metrics directory does not exist: ", metrics_dir))
  }
  if (!file.exists(manual_freq_path)) {
    fail(paste0("Manual frequency TSV does not exist: ", manual_freq_path))
  }
  ...
  exclude <- c(
    "summary_combined_variants.tsv",
    paste0(output_prefix, "_PRNP_PASS.tsv"),
    paste0(output_prefix, "_TET2_PASS.tsv"),
    paste0(output_prefix, "_TTN_PASS.tsv"),
    paste0(output_prefix, "_PRNP_final.tsv"),
    paste0("final_", output_prefix, "_variants.tsv"),
  ...
  summary_table <- NULL
  input_suffix <- paste0("\\.", output_prefix, "\\.tsv$")
  for (file in file_list) {
    ...
    temp_table <- temp_table %>%
      rename_with(
        .fn = ~ str_remove(., "^.*\\."),
        .cols = ends_with(c(".GT", ".DP", ".AD", ".F1R2", ".F2R1", ".SB"))
      )
    if (nrow(temp_table) == 0L) next

    sample_name <- str_remove(basename(file), input_suffix)
    sample_name <- str_remove(sample_name, "\\.tsv$")
    colnames(temp_table) <- gsub(
      paste0("^", sample_name, "\\."),
      "sample.",
      colnames(temp_table)
    )
  ...
  required_cols <- c(
    "sample", "CHROM", "POS", "REF", "ALT", "FILTER", "FUNCOTATION",
    "DP", "AD", "SB"
  )
  missing_cols <- setdiff(required_cols, colnames(summary_table))
  if (length(missing_cols) > 0L) {
    fail(paste0(
      "Variant table is missing required column(s): ",
  ...
  funcotation_primary <- strsplit(
    coalesce(summary_table$FUNCOTATION, ""),
    ",",
    fixed = TRUE
  )
  funcotation_primary <- vapply(
    funcotation_primary,
    function(x) if (length(x) == 0L) "" else x[[1]],
    character(1)
  )
  ...
  summary_table <- summary_table %>%
    ...
    mutate(
      ...
      mutation_type = str_extract(
        FUNCOTATION_PRIMARY,
        paste0(
          "\\b(SILENT|INTRON|MISSENSE|IGR|INTERGENIC|UPSTREAM|DOWNSTREAM|",
          "FIVE_PRIME_UTR|THREE_PRIME_UTR|SPLICE_SITE|SPLICE_DONOR|",
          "SPLICE_ACCEPTOR|FRAME_SHIFT|STOP_GAINED|STOP_LOST|START_LOST|",
          "START_GAINED|SYNONYMOUS|NONSENSE|INFRAME_DELETION|",
          "INFRAME_INSERTION|COMPLEX_SUBSTITUTION|TRANSCRIPT_ABLATION|",
  ...
  manual_by_dbsnp <- manual_freq %>%
    ...
    select(
      dbsnp_id,
      population_frequency_gnomAD_manual_dbsnp = population_frequency
    ) %>%
  ...
  manual_by_variant <- manual_freq %>%
    ...
    select(
      variant_id,
      population_frequency_gnomAD_manual_variant = population_frequency
    ) %>%
  ...
  for (i in seq_along(metric_files)) {
    file <- metric_files[[i]]
    sample_name <- sub("_metrics\\.tsv$", "", basename(file))
    temp <- read_tsv(
      file,
      col_types = cols(
        CHROM = col_character(),
  ...
  if (enable_aaf_filter) {
    ...
    step5 <- step4
    n5 <- n4
    step5_label <- if (output_prefix == "noPoN") {
      paste0("AAF filter disabled (threshold would be > ", aaf_threshold, ")")
    } else {
      "AAF filter disabled"
    }
  }
  filter_counts <- tibble(
    step = c(
      ...
      paste0(
        "strand balance >= ", min_strand_alt, " ALT reads per strand"
      ),
      paste0(
        "MEAN_BQ >= ", min_mean_bq, ", MEAN_MQ >= ", min_mean_mq,
        ", binomial p <= ", max_binom_p
      ),
  ...
  if (is.null(sample_manifest_path)) {
    settings <- tibble(
      key = c(
        ...
        "min_strand_alt", "min_mean_bq", "min_mean_mq", "max_pop_freq",
        "max_binom_p"
      ),
      value = c(
        ...
        as.character(max_binom_p)
      )
    )
  } else {
    settings <- tibble(
      key = c(
        "enable_aaf_filter", "aaf_filter_applied", "aaf_threshold",
        "min_alt_count", "min_dp", "min_strand_alt", "min_mean_bq",
        "min_mean_mq", "max_pop_freq", "max_binom_p", "sample_manifest"
      ),
      value = c(
  ...
  cat(
    "Summary table:  ",
    file.path(output_dir, "summary_combined_variants.tsv"),
    "\n",
    sep = ""
  )
  cat(
    "Filtered table: ",
    file.path(output_dir, "filtered_variants.tsv"),
    "\n",
    sep = ""
  )
  cat(
    "Filter counts:  ",
    file.path(output_dir, "filter_counts.tsv"),
    "\n",
    sep = ""
  )
}
