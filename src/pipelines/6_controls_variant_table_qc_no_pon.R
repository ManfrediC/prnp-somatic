#!/usr/bin/env Rscript
# ============================================================
# Controls-only variant extraction + QC (no PoN)
# Adapted from legacy createTable.R with reproducible CLI I/O.
# ============================================================
...
exclude <- c(
  "summary_combined_variants.tsv", "noPoN_PRNP_PASS.tsv", "noPoN_TET2_PASS.tsv",
...
for (file in file_list) {
  ...
  sample_name <- str_remove(sample_name, "\\.noPoN\\.tsv$")
...
write_tsv(summary_table %>% filter(gene == "TTN"), file.path(output_dir, "noPoN_TTN_PASS.tsv"), na = "")
