...
VARIANT_QC_OUTPUT_DIR="${VARIANT_QC_OUTPUT_DIR:-results2/sequencing2/results/mutect2_controls_no_pon/variant_qc}"

VARIANT_QC_R_SCRIPT="${VARIANT_QC_R_SCRIPT:-src2/sequencing2/6_controls_variant_table_qc_no_pon.R}"
...
if [[ "$DRY_RUN" == "0" ]]; then
  ...
  grep -Fqx $'aaf_threshold\t0.006682086867129272' "$VARIANT_QC_OUTPUT_DIR/run_settings.tsv" ||
    die "Final settings do not record the empirical LoD"
fi
